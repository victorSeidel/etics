...
import { createAsaasCustomer, createAsaasSubscription, cancelAsaasSubscription } from '../services/asaasService.js';
...
router.get("/plans", async (req, res) =>
{
    try
    {
        const result = await pool.query(`SELECT id, name, display_name, description, credits_per_month, price_monthly, price_yearly, features FROM plans
...
router.get("/all", authenticateToken, requireAdmin, async (req, res) =>
{
    try
    {
        const result = await pool.query(`SELECT s.*, p.name as plan_name, p.display_name as plan_display_name, p.description as plan_description, p.credits_per_month, 
            p.price_monthly, p.features FROM subscriptions s JOIN plans p ON s.plan_id = p.id`);
...
router.get("/my-subscription", authenticateToken, async (req, res) =>
{
    ...
    {
        const result = await pool.query(`SELECT s.*, p.name as plan_name, p.display_name as plan_display_name, p.description as plan_description, p.credits_per_month, 
            p.price_monthly, p.features FROM subscriptions s JOIN plans p ON s.plan_id = p.id WHERE s.user_id = $1 AND s.status = 'active'`, [req.user.id]);
...
router.post("/change-plan", authenticateToken, async (req, res) =>
{
    ...
    try
    {
        ...
        if (!['monthly', 'yearly'].includes(cycle)) return res.status(400).json({ error: "Ciclo de cobrança inválido" });
        ...
        const period = cycle === 'yearly' ? '1 year' : '30 days';
...
router.post("/subscribe", authenticateToken, async (req, res) =>
{
    ...
    try
    {
        const { planId, billingCycle, paymentData } = req.body;

        if (!planId || !paymentData) return res.status(400).json({ error: "Dados incompletos" });

        const { nomeTitular, cpf, email, telefone, cep, numeroCasa, numeroCartao, validade, cvv } = paymentData;

        if (!nomeTitular || !cpf || !email || !telefone || !cep || !numeroCasa || !numeroCartao || !validade || !cvv) 
        {
            return res.status(400).json({ error: "Todos os campos de pagamento são obrigatórios" });
        }
        ...
        //const planValue = cycle === 'yearly' ? plan.price_yearly : plan.price_monthly;
        const planValue = plan.price_monthly;
        ...
        const periodEnd = cycle === 'yearly' ? '1 year' : '30 days';
        ...
        let subscriptionResult = await client.query(`UPDATE subscriptions SET plan_id = $2, credits_available = credits_available + $3, billing_cycle = $4, 
            current_period_end = CURRENT_TIMESTAMP + INTERVAL '${periodEnd}', status = 'active', asaas_subscription_id = $5, payment_method = 'CREDIT_CARD',
            next_due_date = $6, cancelled_at = NULL
                WHERE user_id = $1 RETURNING *`, [req.user.id, planId, plan.credits_per_month, cycle, asaasSubscription.id, asaasSubscription.nextDueDate]);
        ...
        if (subscriptionResult.rows.length === 0) 
        {
            subscriptionResult = await client.query(`INSERT INTO subscriptions (user_id, plan_id, credits_available, billing_cycle, current_period_end, status, 
                asaas_subscription_id, payment_method, next_due_date) VALUES ($1, $2, $3, $4, CURRENT_TIMESTAMP + INTERVAL '${periodEnd}', 'active', $5, 'CREDIT_CARD', $6)`, 
                    [req.user.id, planId, plan.credits_per_month, cycle, asaasSubscription.id, asaasSubscription.nextDueDate]);
        }

        subscription = subscriptionResult.rows[0];

        await client.query(`INSERT INTO credit_transactions (user_id, subscription_id, amount, type, description)
            VALUES ($1, $2, $3, 'credit', $4)`, [req.user.id, subscription.id, plan.credits_per_month, `Créditos do plano ${plan.display_name}`]);

        await client.query('COMMIT');

        res.json({
            message: "Assinatura criada com sucesso",
            subscription: { ...subscription, plan_name: plan.name, plan_display_name: plan.display_name, asaas_subscription_id: asaasSubscription.id }
        });
    }
    catch (error)
    {
