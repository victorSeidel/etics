...
export async function createAsaasSubscription(subscriptionData) {
    try {
        ...
        const payload =
        {
            ...
            nextDueDate: subscriptionData.nextDueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
        ...
        if (subscriptionData.billingType === 'CREDIT_CARD') {
            if (!subscriptionData.creditCard) throw new Error('Dados do cartão de crédito são obrigatórios para pagamento com cartão');
